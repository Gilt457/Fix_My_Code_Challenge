...
class User:
    ...
    def password(self, value):
        ...
        - `None` if `value` is not a string
        - Hash `value` in MD5 before assign to `__password`
        """
        if value is None or not isinstance(value, str):
            self.__password = None
        else:
            self.__password = hashlib.md5(value.encode()).hexdigest().lower()
    ...
    def is_valid_password(self, value):
        ...
        - Compare `__password` and the MD5 value of `value`
        """
        if value is None or not isinstance(value, str):
            return False
        if self.password is None:
            return False
        return self.password == hashlib.md5(value.encode()).hexdigest().lower()
